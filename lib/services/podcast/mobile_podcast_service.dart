...
class MobilePodcastService extends PodcastService {
  ...
  @override
  Future<psearch.SearchResult?> mostRecent() => api!.mostRecent();
  ...
  Future<Podcast?> loadPodcast({
    ...
    if (podcast.id == null || refresh!) {
      ...
      Podcast? pc = Podcast(
        ...
        episodes: <Episode>[],
        metadata: metadata,
        value: Value.fromPodcastSearchValue(loadedPodcast.value),
      );
      ...
      if (loadedPodcast.episodes != null) {
        ...
        for (final episode in loadedPodcast.episodes!) {
          ...
          if (episodesMetadataItems != null) {
            episodeMetadata = episodesMetadataItems.firstWhereOrNull((item) {
              if (item['guid'] != null) {
  ...
  @override
  Future<FileSystemEntity?> deleteDownload(Episode episode) async {
  ...
  @override
  Future<Episode> toggleEpisodePlayed(Episode episode) async {
    episode.played = !episode.played;
    episode.position = 0;

    return await repository.saveEpisode(episode);
  }
